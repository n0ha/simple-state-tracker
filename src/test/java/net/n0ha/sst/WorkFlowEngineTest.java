...
import static net.n0ha.sst.MockButton.APPROVE;
...
public class WorkFlowEngineTest extends UnitTestingSupport {
	...
	public void testCorrectTransitionIsFoundForUser() throws Exception {
		...
		wf.addTransition(t1);
		wf.addTransition(t2);
		wf.addTransition(new Transition(NEW, PROCESSED, APPROVE, USER));

		// choose transitions
		List<Transition> transitions = wf.entity(request).canMoveTo();

		// only one transition must be chosen
		assertEquals(1, transitions.size());

		// the one from Start to New
		assertEquals(t1, transitions.get(0));

		// button for transition
		assertEquals(SAVE, transitions.get(0).getButton());

		// button from workflow buttons
		assertEquals(wf.getButtons(request).get(0), transitions.get(0).getButton());

		// temporarily current role is changed to APPROVER
		List<Transition> transitionsAsApprover = wf.entity(request, APPROVER).canMoveTo();

		// only one transition must be chosen
		assertEquals(1, transitionsAsApprover.size());

		// the one from Start to New as APPROVER
		assertEquals(t2, transitionsAsApprover.get(0));

		// button from workflow buttons should not change
		assertEquals(SAVE, wf.getButtons(request).get(0));

		List<Transition> transitionsAfter = wf.entity(request).canMoveTo();

		// the one from Start to New
		assertEquals(t1, transitionsAfter.get(0));
	}

	public void testCorrectTransitionIsFoundForApprover() throws Exception {

		// configure the engine
		WorkFlowEngine wf = getBasicFlowForApprover();

		Transition t1 = new Transition(START, NEW, SAVE, USER);
		Transition t2 = new Transition(START, NEW, APPROVE, APPROVER);
		wf.addTransition(t1);
		wf.addTransition(t2);
		wf.addTransition(new Transition(START, PROCESSED, SAVE, APPROVER));

		// choose transitions
		List<Transition> transitions = wf.entity(request).canMoveTo();

		// 2 transitions must be chosen
		...
		// button for transition
		assertEquals(APPROVE, transitions.get(0).getButton());
		assertEquals(SAVE, transitions.get(1).getButton());

		// 2 buttons must be chosen
		assertEquals(2, wf.getButtons(request).size());

		// button from workflow buttons
		assertEquals(wf.getButtons(request).get(0), transitions.get(0).getButton());
		assertEquals(wf.getButtons(request).get(1), transitions.get(1).getButton());
	}
	...
	public void testNonExistantPathForTemporaryRoleThrowsException() throws Exception {
		...
		wf.addTransition(new Transition(NEW, PROCESSED, APPROVE, USER));
		try {
			wf.entity(request, APPROVER).to(SAVE);
			fail("Non existant path must throw IllegalStateException");
		} catch (IllegalStateException e) {
			// pass
		}
	...
	public void testNullRoleThrowsException() throws Exception {
		...
		wf.addTransition(new Transition(NEW, PROCESSED, APPROVE, USER));
		try {
			wf.entity(request, null).to(SAVE);
	...
	public void testCallbackIsExecuted() throws Exception {
		...
		assertTrue(flow.entity(request).to(SAVE));
	...
	public void testSubFlowIsImportedCorrectly() throws Exception {
		...
		flow.entity(request).to(MockButton.APPROVE);
		...
		flow.entity(request).to(MockButton.CANCEL);
	...
	public void testCalculatesFinalStatesCorrectly() throws Exception {
		...
		flow.addTransition(new Transition(MockState.START, MockState.APPROVED, MockButton.SAVE, MockRole.USER));
	...
	public void testRecursionFromCallback() throws Exception {
		...
		t.addCallback(new Callback() {
			public boolean execute(FlowEntity entity, Map<String, Object> params) {
				flow.entity(entity, MockRole.SYSTEM).to(APPROVE);
				return false;
			}
		});
		...
		flow.entity(request).to(MockButton.SAVE);
